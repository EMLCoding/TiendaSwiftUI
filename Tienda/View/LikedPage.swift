...
struct LikedPage: View {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack {
                    HStack {
                        ...
                        .opacity(sharedDataVM.likedProducts.isEmpty ? 0 : 1)
                    }
                    
                    if sharedDataVM.likedProducts.isEmpty {
                        Group {
                            Text("No hay favoritos")
                                .font(.title)
                                .bold()
                            
                            Text("Pulsa el botón de favoritos de un producto para que aparezca en esta pantalla")
                                .font(.title3)
                        ...
                        VStack(spacing: 15) {
                            ForEach(sharedDataVM.likedProducts) { product in
                                HStack(spacing: 0) {
                                    if showDeleteOption {
                                        Button {
    ...
    func deleteProduct(product: Product) {
        if let index = sharedDataVM.likedProducts.firstIndex(where: { currentProduct in
